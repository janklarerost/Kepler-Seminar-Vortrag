...
\begin{frame}{1. Keplersches Gesetz}
    ...
    \begin{tikzpicture}[scale=0.9]
        ...
        \coordinate (Sonne) at (1,0);
        \coordinate (Perihel) at (2.5,0); % Hilfspunkt für den Winkel
        \coordinate (Planet) at (50:2cm and 1.2cm);
        
        % Achsen/Hilfslinien
        \draw[dashed, gray] (-2.5,0) -- (2.5,0);
        
        % Ellipse
        \draw[thick] (0,0) ellipse (2cm and 1.2cm);
        
        % Brennpunkte
        \filldraw[orange] (Sonne) circle (3pt) node[below=2pt] {Sonne};
        \filldraw[black] (-1,0) circle (1pt) node[below=2pt] {$F_2$};
        
        % Planet
        \filldraw[blue] (Planet) circle (2pt) node[above right] {Planet};
        
        % Radius vector
        \draw[thick, ->] (Sonne) -- (Planet) node[midway, above left] {$r$};
        
    
       \pic[draw, thick, ->, angle eccentricity=1.5, angle radius=0.6cm, pic text={$\vartheta$}] {angle = Perihel--Sonne--Planet};
    \end{tikzpicture}
    \end{center}
    ...
    \begin{itemize}
        ...
        \item $\varepsilon$: Numerische Exzentrizität (für Ellipsen: $0 < \varepsilon < 1$)
    \end{itemize}
    ...
    \note{
        ...
        Für unsere Berechnungen reicht es, wenn wir uns auf Polarkoordinaten beschränken: Wir schauen uns den Radius $r$ (den Abstand zur Sonne) und den Winkel $\vartheta$ an. Die 'Ovalheit' der Bahn wird durch die numerische Exzentrizität $\varepsilon$ beschrieben. Mehr Geometrie brauchen wir heute gar nicht.
    }
\end{frame}

% --- Slide 3: 2. Keplersches Gesetz ---
\begin{frame}{2. Keplersches Gesetz (Flächensatz)}
    Der Fahrstrahl (Sonne $\rightarrow$ Planet) überstreicht in gleichen Zeiten gleich große Flächen.
    \vspace{0.7cm}
    
    Physikalisch bedeutet dies die \textbf{Drehimpulserhaltung}. Für uns relevant ist nur die Aussage zur Winkelgeschwindigkeit $\dot{\vartheta}$:
    
    \begin{block}{Zusammenhang Winkelgeschwindigkeit und Radius}
        \centering
        \Large
        $L = r \cdot v \cdot m = r^2 \cdot \dot{\vartheta} \cdot m = \operatorname{const}$
    \end{block}
    ...
    \begin{itemize}
        ...
        \item $\dot{\vartheta} = \frac{d\vartheta}{dt}$: Winkelgeschwindigkeit
    \end{itemize}
    \textit{Fazit: Die Winkelgeschwindigkeit hängt direkt vom aktuellen Radius ab!}
    
    \note{
        Das 2. Keplersche Gesetz ist der berühmte Flächensatz. Physikalisch steckt dahinter die Erhaltung des Drehimpulses $L$.\\
        Wir haben Ihnen hier die wesentliche Formel mitgebracht. Da der Drehimpuls konstant ist, sehen wir sofort unseren wichtigsten physikalischen Zusammenhang für heute: Die Winkelgeschwindigkeit $\dot{\vartheta}$ hängt direkt vom aktuellen Radius ab. Ist der Planet nah an der Sonne, ist $r$ klein, also muss die Winkelgeschwindigkeit groß sein.
    }
\end{frame}
...
\begin{frame}{Gilt das auch für elliptische Bahnen?}
    Kreisbahnen sind ein Idealfall. Was passiert bei starken Abweichungen?
    \vspace{0.4cm}
    
    \textbf{Beispiel Mars:} (Stärkste Exzentrizität der inneren Planeten, $\varepsilon \approx 0.093$)
    \begin{itemize}
...
\begin{frame}{Berechnung der Umlaufzeit}
    ...
    Mit der Parametrisierungsformel für Ellipsen $\displaystyle r(\vartheta) = \frac{p}{1 + \varepsilon \cos(\vartheta)}$:
    
    \begin{block}{Das T-Integral}
        \Large
        \[ T = \frac{m \cdot p^2}{L} \int_0^{2\pi} \frac{1}{(1 + \varepsilon \cos(\vartheta))^2} \, d\vartheta \]
    \end{block}
...
\begin{frame}{Die wichtigste Idee: Der Weg in die komplexe Ebene}
    ...
    \begin{block}{Das komplexe Wegintegral}
        \Large
        \[ \int_0^{2\pi} \frac{1}{(1 + \varepsilon \cos(\vartheta))^2} \, d\vartheta \quad\Rightarrow\quad \frac{4}{i \varepsilon^2} \oint_C \frac{z}{\left(z^2 + \frac{2}{\varepsilon}z + 1\right)^2} \, dz \]
    \end{block}
...
\begin{frame}{Analyse der Polstellen}
    ...
    \[ z_{\pm} = -\frac{1}{\varepsilon} \pm \sqrt{\frac{1}{\varepsilon^2} - 1} \]
    ...
    \note{
        ...
        Da $\varepsilon$ kleiner als 1 ist, liegt $z_-$ weit außerhalb, aber $z_+$ liegt *innerhalb* unseres Einheitskreises. Und nur dieser Punkt liefert einen Beitrag zu unserem Integral.
    }
\end{frame}
...
\begin{frame}{Auswertung der Ableitung}
    ...
    \[ f'(z_+) = \frac{\varepsilon^2}{4(1-\varepsilon^2)^{\frac{3}{2}}} \]
    
    Eingesetzt in das ursprüngliche $T$-Integral (inklusive Vorfaktor):
    
    \begin{align*}
        \int_0^{2\pi} \frac{1}{(1 + \varepsilon \cos(\vartheta))^2} \, d\vartheta &= \frac{4}{i \varepsilon^2} \cdot 2\pi i \cdot \frac{\varepsilon^2}{4(1-\varepsilon^2)^{\frac{3}{2}}} = \frac{2\pi}{(1-\varepsilon^2)^{\frac{3}{2}}}
    \end{align*}
...
\begin{frame}{Auflösung: Das 3. Keplersche Gesetz}
    ...
    \[ T = \frac{m \cdot p^2}{L} \cdot \frac{2\pi}{(1-\varepsilon^2)^{\frac{3}{2}}} \quad\quad \text{mit } p = a(1-\varepsilon^2) \text{ und } L = m \sqrt{GMp} \]
    
    Setzt man $L$ und $p$ konsequent ein, vereinfacht sich der Ausdruck:
    \begin{align*}
        T &= \frac{2\pi \cdot p^{\frac{3}{2}}}{\sqrt{GM} \cdot (1-\varepsilon^2)^{\frac{3}{2}}} 
           = \frac{2\pi \cdot a^{\frac{3}{2}} \cdot (1-\varepsilon^2)^{\frac{3}{2}}}{\sqrt{GM} \cdot (1-\varepsilon^2)^{\frac{3}{2}}} 
